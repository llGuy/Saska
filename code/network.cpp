...
void bind_network_socket_to_port(network_socket_t *socket, network_address_t address)
{
    ...
    address_struct.sin_port = htons(address.port);
...
void send_to(network_socket_t *socket, network_address_t address, char *buffer, uint32_t buffer_size)
{
    ...
    int32_t address_size = sizeof(address_size);
    
    int32_t sendto_ret = sendto(*sock, buffer, buffer_size, 0, (SOCKADDR *)&address_struct, address_size);
...
void initialize_as_client(void)
{
    ...
    address.port = g_network_state->GAME_OUTPUT_PORT;
...
{
    client_joint_packet_t packet_to_send = {};
